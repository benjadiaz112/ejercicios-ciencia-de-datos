...
# 2. VARIABLES DE ENTRADA (X) Y SALIDA (y)
...
# 4. CREACIÓN Y ENTRENAMIENTO DEL ÁRBOL
# Usamos max_depth para que no sea un árbol infinito y sea fácil de leer
...
# 6. PREDICCIÓN DE NUEVO ESTUDIANTE
# Definimos el nuevo estudiante como un DataFrame para evitar la advertencia (Warning)
nuevo_estudiante = pd.DataFrame([[5, 80, 4, 5.2]], columns=X.columns)
pred = modelo.predict(nuevo_estudiante)

print("\n--- Resultado Nuevo Estudiante ---")
print(f"Predicción: {'Aprueba' if pred[0] == 1 else 'No Aprueba'}")
